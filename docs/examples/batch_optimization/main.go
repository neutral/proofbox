...
func main() {
	...
	store, cleanup, err := utils.CreateTempStorage("batch-optimization")
	if err != nil {
	...
	if err != nil {
		utils.PrintError("Failed to get stats: %v", err)
	} else {
	...
	if err != nil {
		...
		if err != nil {
			utils.PrintError("Decompression failed: %v", err)
		} else {
			utils.PrintSuccess("Compression/decompression cycle successful!")
...
// To run this example:
// cd docs/examples/batch_optimization && go run .
// Or: cd docs/examples cd examples && ./run_example.shcd examples && ./run_example.sh ./run_example.sh batch_optimization