...
func TestExclusionEmptyProofInNonEmptyTree(t *testing.T) {
	...
	version, _ := tr.Put(types.KeyHash([]byte("key1")), []byte("value1"))
	tr.Put(types.KeyHash([]byte("key3")), []byte("value3"))
	version, _ = tr.Put(types.KeyHash([]byte("key5")), []byte("value5"))
...
func TestExclusionNeighborProof(t *testing.T) {
	...
	tr.Put(key1, []byte("value1"))
	version, _ := tr.Put(key3, []byte("value3"))
...
func TestProofTampering(t *testing.T) {
	...
	version, _ := tr.Put(key, value)
	reader, _ := tr.Reader(version)
	defer reader.Close()
...
func TestBatchProofs(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		tr.Put(keys[i], []byte(fmt.Sprintf("value-%d", i)))
	}
