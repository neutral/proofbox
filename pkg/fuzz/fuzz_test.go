...
func verifyTreeInvariants(tr interface{ GetRootHash(types.Version) (types.Hash, error); GetLatestVersion() types.Version }) error {
	// Basic invariant: root hash should be deterministic
	version := tr.GetLatestVersion()
	rootHash1, _ := tr.GetRootHash(version)
	rootHash2, _ := tr.GetRootHash(version)
